    ...
    <div class="cart-info">
      ...
      <div class="cart-item" v-for="course in course_list">
        <el-row>
          ...
          <el-col :span="10" class="course-info">
            ...
            <span class="course_name">{{ course.name }}<br><span
              class="discount">{{ course.discount_name }}</span></span>
          </el-col>
      ...
      <div class="discount">
        <div id="accordion">
          ...
          <div id="collapseOne" v-if="use_coupon">
            <ul class="coupon-list" v-if="coupon_list.length>0">
              ...
              <!--                  <p class="coupon-time end_time">过期时间：2019-11:01 00:00:00</p>-->
              <!--                </li>-->
              <li class="coupon-item" v-for="(item,key) in coupon_list " :class="selected_coupon(key,item.id)"
                  @click="click_select_coupon(key,item.id)">
        ...
        <div class="credit-box">
          ...
          </el-input-number>，本次花费以后，剩余{{ parseInt(user_credit - credit) }}积分</span></p>
        </div>
      </div>
      <div class="calc">
        <el-row class="pay-row">
          ...
          <el-col :span="8">
            <span class="alipay" v-if="pay_type===1"><img src="../../static/image/alipay2.png" alt=""></span>
            <span class="alipay" @click="pay_type=1" v-else><img src="../../static/image/alipay.png" alt=""></span>
            <span class="alipay wechat" v-if="pay_type===2"><img src="../../static/image/wechat2.png" alt=""></span>
            <span class="alipay wechat" @click="pay_type=2" v-else><img src="../../static/image/wechat.png"
                                                                        alt=""></span>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    max_credit(){
        ...
        // 用户拥有的积分允许抵扣的最大金额
        let max_credit_to_money = parseInt(this.user_credit) / parseInt(this.credit_to_money);
        console.log( "用户积分抵扣",max_credit_to_money );
        // 计算当前真实的订单实付金额
        let ret = 0;
        if(max_credit_to_money > this.real_total){
            ret = parseInt(this.real_total) * parseInt(this.credit_to_money);
        }else{
