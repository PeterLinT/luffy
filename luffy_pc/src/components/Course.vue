    ...
    <div class="main">
      ...
      <div class="condition">
        <ul class="cate-list">
          ...
          <li @click="category=cat.id" :class="category==cat.id?'this':''" v-for="cat in category_list">{{ cat.name }}
          </li>
        </ul>
        ...
        <div class="course-item" v-for="course in course_list">
          ...
          <div class="course-info">
            ...
            <p class="teather-info">{{ course.teacher.name }} {{ course.teacher.title }} {{
              ...
              }}<span>共{{ course.lessons }}课时/更新完成</span></p>
            <ul class="lesson-list">
              <li v-for="lesson in course.lessons_list"><span class="lesson-title">01 | 第{{
                  lesson.time
                }}节：{{ lesson.name }}</span> <span class="free" v-if="lesson.free_trail">免费</span></li>

            </ul>
            <div class="pay-box">
              <span class="discount-type" v-if="course.real_price < course.price">限时折扣</span>
              <span class="discount-price">￥{{ course.real_price}}元</span>
              <span class="original-price" v-if="course.real_price < course.price">原价：{{ course.price }}元</span>
...
export default {
  ...
  data() {
    return {
      ...
      filters: {
        ...
        orders: "",  // 排序类型，desc表示降序，asc表示升序,-表示降序，""表示升序
        size: 5, // 单页数据量
        page: 1, // 当前页码
      },
      course_list: [],
  ...
  methods: {
    ...
    change_order_type(type) {
      // 更改升序或者降序
      if (this.filters.type === type && this.filters.orders === "-") {
        this.filters.orders = "";
      } else if (this.filters.type === type && this.filters.orders === "") {
        this.filters.orders = "-";
      }
      // 更改排序方式
    ...
    get_course() {
      ...
      filter.ordering = this.filters.orders + this.filters.type
      if (this.category > 0) {
        filter.course_category = this.category
      }

      console.log(filter)
      this.$axios.get(`${this.$settings.HOST}/course/`, {params: filter})
        .then(response => {
